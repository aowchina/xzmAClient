...
//    protected int[] itemdrawables = { R.drawable.ease_chat_takepic_selector, R.drawable.ease_chat_image_selector };
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        img = fragmentArgs.getString("userHeadImage");
        if(fragmentArgs.getString("userId")!= null) {
            toChatUsername = fragmentArgs.getString("userId").toLowerCase();
        }
    ...
    protected void initView() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            RelativeLayout rlTitle = (RelativeLayout) getView().findViewById(R.id.titles);
            if(rlTitle!=null) {
                rlTitle.setPadding(0, dip2px(getActivity(), 22), 0, 0);
            }
        }
        ...
        tvRight = (TextView) getView().findViewById(R.id.tv_right);
...
//        titleBar.setTitle(userNickName);
...
//                            titleBar.setTitle(toChatUsername);
    ...
    public void onMessageReceived(List<EMMessage> messages) {
        for (EMMessage message : messages) {
            ...
            LG.e("聊天列表", "username="+username);
            LG.e("聊天列表", "toChatUsername="+toChatUsername);
            LG.e("聊天列表", "message.getTo()="+message.getTo());
            if (username.equals(toChatUsername.toLowerCase()) || message.getTo().equals(toChatUsername)) {
...
//                                    Toast.makeText(getActivity(), str,Toast.LENGTH_SHORT).show();
                                    //权限被拒绝
                                    EaseDialogUtil.showMsgDialog(getActivity(), "提示", "需要开启相机权限后才能使用", "去设置", "取消", new EnsureCancleInterface() {
                                        @Override
                                        public void ensure() {
                                            Intent intent= CMethodUtil.getAppDetailSettingIntent(getActivity());
...
//                                    Toast.makeText(getActivity(), str,Toast.LENGTH_SHORT).show();
                                    //权限被拒绝
                                    EaseDialogUtil.showMsgDialog(getActivity(), "提示", "需要开启存储权限后才能使用", "去设置", "取消", new EnsureCancleInterface() {
                                        @Override
                                        public void ensure() {
                                            Intent intent= CMethodUtil.getAppDetailSettingIntent(getActivity());
                                            startActivity(intent);
                    ...
                    requestRunTimePermission(new String[]{Manifest.permission.ACCESS_FINE_LOCATION}
                            , new PermissionListener1() {
                                ...
                                public void onDenied(List<String> deniedPermission) {
                                    ...
                                    for(String string : deniedPermission) {
                                        if(string.equals(Manifest.permission.ACCESS_FINE_LOCATION)) {
                                            str += "定位";
                                        }
                                    }
                                    str += "权限";
                                    Toast.makeText(getActivity(), str,Toast.LENGTH_SHORT).show();
                                    //权限被拒绝
                                    EaseDialogUtil.showMsgDialog(getActivity(), "提示", "需要开启定位权限后才能使用", "去设置", "取消", new EnsureCancleInterface() {
                                        @Override
                                        public void ensure() {
                                            Intent intent= CMethodUtil.getAppDetailSettingIntent(getActivity());
                                            startActivity(intent);
